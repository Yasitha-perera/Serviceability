...
<form method='post' action='save_serviceability.php'> 
  ...
  $sql = "SELECT * FROM vhf_communication WHERE stid = $loggedInUserStid AND cid = 1 AND loid='Control Tower'";
  ...
  if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
          ...
          echo "<td>" . $row["equipment"] ."&nbsp;/&nbsp;".$row["transmits"] ."&nbsp;/&nbsp;".$row["status"] . "</td>";
          echo "<td>" . $row["loid"] . "</td>";
          echo "<td>";
          echo "<input type='hidden' name='mid[]' value='" . $row['vhfid'] . "'>"; 
          echo "<input type='hidden' name='equipment[]' value='" . $row["equipment"] ."&nbsp;/&nbsp;".$row["transmits"] ."&nbsp;/&nbsp;".$row["status"] . "'>"; 
          echo "<input type='hidden' name='loid[]' value='" . $row['loid'] . "'>";
          echo "<input type='hidden' name='cid[]' value='1'>"; 
          echo "<input type='hidden' name='machine_type[]' value='vhf_communication'>"; 
...
$sql = "SELECT * FROM machine WHERE stid = $loggedInUserStid AND cid = 1 AND machine_type = 'scanners' AND loid='Control Tower'";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<input type='hidden' name='loid[]' value='" . $row['loid'] . "'>";
        echo "<input type='hidden' name='cid[]' value='1'>"; 
        echo "<input type='hidden' name='machine_type[]' value='scanners'>"; 
        echo "<input type='hidden' name='stid[]' value='" . $loggedInUserStid . "'>"; // Add stid as hidden input
        echo "<select name='serviceability[]' class='form-control'>";
        echo "<option value='0' selected>Select</option>"; 
...
$sql = "SELECT * FROM machine WHERE stid = $loggedInUserStid AND cid = 1 AND machine_type = 'links' AND loid='Control Tower'";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<input type='hidden' name='loid[]' value='" . $row['loid'] . "'>";
        echo "<input type='hidden' name='cid[]' value='1'>"; 
        echo "<input type='hidden' name='machine_type[]' value='links'>"; 
        echo "<input type='hidden' name='stid[]' value='" . $loggedInUserStid . "'>"; // Add stid as hidden input
        echo "<select name='serviceability[]' class='form-control'>";
        echo "<option value='0' selected>Select</option>"; 
